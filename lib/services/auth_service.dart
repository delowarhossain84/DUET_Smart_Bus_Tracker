...
class AuthService { // Authentication এবং backend related কাজ রাখার service class।
      ...
      try {
        ...
        if (e.toString().contains("Failed to fetch") || e.toString().contains("ClientException")) {
          ...
          return AuthResponse(
            ...
            message: "Logged in (Server offline/CORS fallback mode).",
            user: user,
          );
        }
        return AuthResponse(
    ...
    if (useRealBackend) {
      try {
        if (role == UserRole.admin) {
          ...
          final payload = AdminRegistrationPayload(
            name: name,
            email: trimmedEmail,
            password: password,
            mobile: phone,
          ...
          print("ADMIN REGISTER STATUS: ${response.statusCode}");
          print("ADMIN REGISTER BODY: ${response.body}");
          ...
          if (response.statusCode == 200 || response.statusCode == 201) {
            ...
            return AuthResponse(
              status: AuthStatus.success,
              message: "Admin account registered successfully! Please log in.",
              user: user,
            );
          }
          ...
          if (response.statusCode == 200 || response.statusCode == 201) {
            ...
            currentUser = user;
            currentUser = null; // Registration does not authenticate the user because this backend does not return an access token here.
            return AuthResponse(
              status: AuthStatus.success,
  ...
  static Future<void> fetchRoutes() async {
    if (!useRealBackend || bearerToken == null) return;
    try {
      final url = Uri.parse("$baseUrl/api/v1/admin/routes?page=1&per_page=100");
      final response = await http.get(url, headers: {"Authorization": "Bearer $bearerToken"});
      
      print("FETCH ROUTES STATUS: ${response.statusCode}");
      
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        
        // Backend JSON structure checks
        List list = [];
        if (data['data'] != null && data['data']['data'] != null) {
          list = data['data']['data'];
        } else if (data['data'] != null && data['data'] is List) {
          list = data['data']; // In case it's a direct list
        }
        
        print("ROUTES FETCHED: ${list.length}");
        _routes = list.map((r) => BusRoute.fromJson(r)).toList();
      } else {
        print("FETCH ROUTES ERROR: ${response.body}");
      }
    } catch (e) {
      print("Fetch routes exception: $e");
    }
  }
  ...
  static Future<bool> createStop({
    ...
    try {
      ...
      print("CREATE STOP STATUS: ${response.statusCode}");
      print("CREATE STOP BODY: ${response.body}");
  ...
  // Creates a new bus route using the existing backend API.
  static Future<bool> createRoute({
    required String name,
    required String code,
    required String description,
    List<Map<String, int>>? stopIds,
  }) async {
    if (!useRealBackend || bearerToken == null) return true;
    ...
    try {
      ...
      print("CREATE ROUTE STATUS: ${response.statusCode}");
  ...
  static Future<bool> createBus({
    ...
    try {
      final url = Uri.parse("$baseUrl/api/v1/admin/buses");
      final response = await http.post(
        url,
        headers: {
          "Authorization": "Bearer $bearerToken",
