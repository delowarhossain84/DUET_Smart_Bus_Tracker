...
class BusService {
  ...
  static Future<List<BusRoute>> fetchAdminRoutes() async {
    if (!AuthService.useRealBackend || AuthService.bearerToken == null) {
      return getMockRoutes();
    }

    try {
      final url = Uri.parse(
        "${AuthService.baseUrl}/api/v1/admin/routes?page=1&per_page=100",
      );
      ...
      throw Exception(
        "Failed to load routes (${response.statusCode}).",
      );
    } catch (e) {
      print("Fetch admin routes error: $e");
      return [];
    }
  }
